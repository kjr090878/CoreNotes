...
class CoreNotesTests: XCTestCase, Fetchable {
    ...
    func testCreateNewCategory() {
        ...
        XCTAssertEqual(newCategory?.name ?? "", "Robots", "Not able to get value for \"name\" using the name property or was not set previously.")
        
        newCategory?.color = UIColor.softGreen()
        
        // run test to make sure the category has a vlaue for "color"
        ...
        fetchEntity("Category", predicates: nil) { (found) -> () in
            ...
            let categoryNames = categories.map({ (c) -> String in return c.valueForKey("name") as? String ?? ""
